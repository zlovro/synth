...
DWORD __SD_Sectors(SD_DEV* dev)
{
    ...
    if (__SD_Send_Cmd(CMD9, 0) == 0)
    {
        ...
        else if (dev->cardtype & SDCT_SD2)
        {
            ...
            // printf("Capacity: %d MiB\n", ((C_SIZE + 1) * 0x200 * 0x400) / (1024 * 1024));
            return (C_SIZE + 1) * 0x400;
            // C_SIZE = csd[]
...
SDRESULTS SD_Init(SD_DEV* dev)
{
    ...
    for (init_trys = 0; init_trys != SD_INIT_TRYS && !ct; init_trys++)
    {
        ...
        {
            // uartPrintf("SD is in idle state\n");
            // SD version 2?
            if (__SD_Send_Cmd(CMD8, 0x1AA) == 1)
            {
                // uartPrintf("SD version = 2\n");
    ...
    if (ct)
    {
        ...
        // printf("Cardtype: %d, sectors: 0x%x\n", dev->cardtype, dev->last_sector);
        #ifdef SD_IO_DBG_COUNT
        dev->debug.read = 0;
        dev->debug.write = 0;
        #endif
...
SDRESULTS SD_Read(SD_DEV* dev, uint8_t* dat, DWORD sector, WORD ofs, WORD cnt)
{
    #if defined(_M_IX86)    // x86
